...
DetectSurfObject::DetectSurfObject(int hessianValue, int minGoodMatches, Logging* logging) {
  ...
  scene_corners = std::vector< Point2f >(4);
...
void DetectSurfObject::GetPosition(cv::Mat image, ObjectPosition* position) {
  keypointsS.clear();
  surf.detect(image,keypointsS);
  ...
  extractor.compute( image, keypointsS, descriptors_scene );
  matches.clear();
  good_matches.clear();
  ...
  for(int i = 0; i< 4; i++)
  {
    (scene_corners[i]).x = obj_transformed_corners[i].x;
    (scene_corners[i]).y = obj_transformed_corners[i].y;
  }
  ...
  position->Center->SetImagePosition(center_x, center_y, image.cols, image.rows);	
  
  _Logging->Log("Detected position X: ", position->Center->GetNormalizedX());
  _Logging->Log("Detected position Y: ", position->Center->GetNormalizedY());
  
  position->Detected = true;
  position->Corner1->SetImagePosition(scene_corners[0].x, scene_corners[0].y, image.cols, image.rows);
  position->Corner2->SetImagePosition(scene_corners[2].x, scene_corners[2].y, image.cols, image.rows);
