...
NewPing distanceSensor(DISTANCE_SENSOR_TRIG, DISTANCE_SENSOR_ECHO, MAX_DISTANCE);
int SwitchCount = 0;
...
long ExecuteCmd(int cmd, int data)
{
  ...
  case CMD_GET_DISTANCESENSOR_MEASURE:
    distanceMeasure = distanceSensor.ping_cm();
...
{  
  short NewDir = (LastEncoder0TickSpeed - LastEncoder1TickSpeed);
  CurrentDir = NewDir + LastDir;
  DirPID.Compute();
  LastDir = NewDir;
