...
void raspiHandleDataChar(char commandId, char length, char dataArray[]) {
      ...
      unsigned char dataUnsigned = dataArray[0];

      switch(commandId)
      {
        case SERVO_HORIZONTAL_BYTE:
          SetHorizontalAngle(dataUnsigned);
          break;
          
        case SERVO_VERTICAL_BYTE:
          SetVerticalAngle(dataUnsigned);
          break;

        case SERVO_SPEED_BYTE:
          setVerticalServoSpeed(dataUnsigned);
          setHorizontalServoSpeed(dataUnsigned);
          break;
...
void raspiHandleReplyShort(char commandId, char length, short data[]) {
      ...
      switch(commandId)
      {
        ...
        case DISTANCE:
          data[0] = TotalEncoderCountA/10;
          data[1] = TotalEncoderCountB/10;
          break;
...
MessageDataProtocol serialCommandRaspi = MessageDataFactory::Create(&Serial, &raspiDataHandler);

void SetupRaspiSerial()
{
  Serial.begin(115200); 
}

void UpdateReadRaspiCommand()
{
